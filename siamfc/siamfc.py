...
from . import ops
from .backbones import vgg_crop,fuse_layer
...
class Net(nn.Module):
    ...
    def __init__(self, backbone, head,fuse):
    ...
    def forward(self, z, x):
        z5,z4 = self.backbone(z,is_z = True)
        x5,x4 = self.backbone(x,is_z = False)
        r5 = self.head(z5,x5)
        r4 = self.head(z4,x4)


        out = torch.cat([r5,r4],dim = 1)# (8,2,15,15)
        out = self.fuse(out)
        return out
...
class TrackerSiamFC(Tracker):
    ...
    def __init__(self, net_path=None, **kwargs):
        ...
        # setup model
        self.net = Net(backbone=vgg_crop(load_ini_weights=False),head=SiamFC(self.cfg.out_scale),fuse = fuse_layer())
    ...
    def init(self, img, box):
        ...
        z = ops.crop_and_resize(img, self.center, self.z_sz,out_size=self.cfg.exemplar_sz,border_value=self.avg_color)
        
        # exemplar features
        self.z = torch.from_numpy(z).to(self.device).permute(2, 0, 1).unsqueeze(0).float()
        self.kernel5, self.kernel4 = self.net.backbone(self.z, is_z=True)
    
    @torch.no_grad()
    def update(self, img, frame=-1, debug=False,gt=None):
        # set to evaluation mode
        # self.net.eval()
        ...
        x5, x4 = self.net.backbone(x, is_z=False)
        responses5 = self.net.head(self.kernel5, x5)
        responses4 = self.net.head(self.kernel4, x4)


        responses = torch.cat([responses5,responses4],dim = 1)
        responses = self.net.fuse(responses)
    ...
    def track(self, img_files, box, visualize=False,debug = False,gt=None):
        ...
        for f, img_file in enumerate(img_files):
            ...
            if f == 0:
                ...
                boxes[f, :] = self.update(img,f,debug,gt)
            times[f] = time.time() - begin

            if visualize:
                ops.show_image(img, boxes[f, :])

        return boxes, times, frame_num / sum(times)
    ...
    def train_over(self, seqs, val_seqs=None,save_dir='/pretrained'):
        ...
        for epoch in range(self.cfg.epoch_num):
            ...
            for it, batch in enumerate(dataloader):
                ...
                print('Epoch: {} [{}/{}] Loss: {:.5f} lr:{:.5f}'.format(epoch + 1, it + 1, len(dataloader), loss,self.optimizer.param_groups[0]['lr']))
                sys.stdout.flush()
            ...
            net_path = os.path.join(
                save_dir, 'siamvgg_e%d.pth' % (epoch + 1))
